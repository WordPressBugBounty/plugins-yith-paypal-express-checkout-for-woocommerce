...
if ( ! class_exists( 'YITH_Gateway_Paypal_EC_BH_Subscriptions' ) ) {
	/**
	 * Class YITH_Gateway_Paypal_EC_BH_Subscriptions
	 */
	class YITH_Gateway_Paypal_EC_BH_Subscriptions extends WC_Payment_Gateway_CC {
		...
		public function __construct() {
			...
			$this->supports           = array(
				...
				'refunds',
				'bh_subscriptions',
				'bh_subscriptions_scheduling',
				'bh_subscriptions_pause',
				'bh_subscriptions_multiple',
				'bh_subscriptions_payment_date',
				'bh_subscriptions_recurring_amount',
			);
			...
			add_action( 'bluehost/subscriptions/pay_renew_order_with_' . $this->id, array( $this, 'renew_subscriptions' ), 10, 2 );
		...
		public function init_form_fields() {
			$this->form_fields = include YITH_PAYPAL_EC_INC . 'admin/gateway-settings.php';
		}
		...
		public function process_order_payment( $order, $payment_response ) {
			...
			if ( 'pending' === strtolower( $payment_status ) ) {
				...
				if ( 'echeck' === $payment_type ) {
					...
					$this->helper->log_add_message( sprintf( esc_html__( 'Echeck expected on %1$s for order %2$d', 'yith-paypal-express-checkout-for-woocommerce' ), date_i18n( wc_date_format(), bh_subscriptions_date_to_time( $payment_response->get_response_payment_parameter( 'EXPECTEDECHECKCLEARDATE' ) ) ), $order->get_id() ) );
					// translators: Placeholder date.
					$status_note = sprintf( esc_html__( 'Echeck expected on %s', 'yith-paypal-express-checkout-for-woocommerce' ), date_i18n( wc_date_format(), bh_subscriptions_date_to_time( $payment_response->get_response_payment_parameter( 'EXPECTEDECHECKCLEARDATE' ) ) ) );
				}
		...
		public static function ipn_handler() {
			...
			do_action( 'bluehost/subscriptions/ipn_handler_before_call_handler' );
			$the_class = apply_filters( 'bluehost/subscriptions/ipn_handler_class_to_call', 'YITH_PayPal_EC_IPN_Handler' );
